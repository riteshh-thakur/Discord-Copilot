...
async function getActiveConfig() {
    ...
    const config = await back4app_1.AgentConfigService.getActiveConfig();
...
async function shouldBotRespond(message) {
    ...
    if (!config)
        return false; // No config = don't respond
    return (0, botService_1.isAllowedChannel)(message.channelId, config.allowedChannelIds || []);
}
async function retrieveKnowledge(userMessage, ragEnabled) {
    ...
    try {
        ...
        const queryEmbedding = await (0, llmService_1.generateEmbedding)(userMessage);
        // Search for similar knowledge chunks
        const knowledgeChunks = await back4app_1.KnowledgeService.searchSimilar(queryEmbedding, constants_1.RAG_CONFIG.MAX_CHUNKS_PER_QUERY);
...
async function assembleContext(message) {
    const config = await getActiveConfig();
    const memory = await back4app_1.MemoryService.getMemory();
...
- Use appropriate formatting for readability`;
    ...
    const cleanedMessage = (0, botService_1.cleanMessage)(message.content);
...
async function updateMemory(userMessage, botResponse, currentSummary) {
    try {
        ...
        if (combined.length > constants_1.MEMORY_CONFIG.MAX_SUMMARY_LENGTH) {
            // Simple compression: keep last N characters
            finalSummary = combined.slice(-constants_1.MEMORY_CONFIG.MAX_SUMMARY_LENGTH);
        }
        // Get existing memory or create new
        const existingMemory = await back4app_1.MemoryService.getMemory();
        if (existingMemory?.objectId) {
            await back4app_1.MemoryService.updateMemory(existingMemory.objectId, finalSummary);
        }
        else {
            await back4app_1.MemoryService.saveMemory(finalSummary);
        }
    }
    catch (error) {
...
async function handleMessage(message, client) {
    try {
        ...
        if (!shouldRespond)
            return;
        // Check rate limiting
        if (!(0, botService_1.checkRateLimit)(message.channelId)) {
        ...
        // Generate response
        const response = await (0, llmService_1.generateResponse)(context);
        // Send response
        if (response) {
